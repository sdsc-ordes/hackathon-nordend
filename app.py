...
if 'connected' in st.session_state or st.sidebar.button("Connect"):
    try:
        ...
        with driver.session() as sess:
            ...
            node_count = sess.execute_read(get_node_count)
            relationship_count = sess.execute_read(get_relationship_count)
            statistics = sess.execute_read(get_statistics)
            node_labels = list(set(sess.execute_read(get_node_labels)))
            ...
            nodes_limit = 10
            st.subheader(f"The {nodes_limit} nodes with most occurrences are:")
            st.write(sess.execute_read(get_top_n_nodes_counts, nodes_limit))

            # List entity (node) names with their properties
            node_properties_labels = []
            label_dict = {}
            for node_label in node_labels:
                node_properties_labels = list(set(sess.execute_read(get_all_node_properties_with_labels, node_label)))
                label_dict[node_label] = node_properties_labels
            ...
            def update_dict_with_descriptions(main_dict, description_list):
                for item in description_list:
                    ...
                    if item_label in main_dict:
                        # Add the description
                        main_dict[item_label]['description'] = description
            
                        # Append the property descriptions to the 'properties' list
                        for prop_desc in properties:
                            for i_e, element in enumerate(main_dict[item_label]['properties']):
                                if element == prop_desc['property_name']:
                                    main_dict[item_label]['properties'][i_e] = main_dict[item_label]['properties'][i_e] + "\n" + prop_desc['property_description']
                return main_dict


            node_labels = sorted(node_labels)
            node_properties_labels = []
            relationships = sess.execute_read(get_relationship_types)
            ...
            for node_label in node_labels:
                G.add_node(node_label, color='blue', size=1000)

            # Add edges (relationships as directed edges between node types)
            all_relationships = []
            for rel in relationships:
                relationship_pairs = sess.execute_read(get_relationship_pairs, rel)
            ...
            with st.spinner('Distilling the graph...'):
                for node_label in node_labels:
                    ...
                    node_properties_labels = list(set(sess.execute_read(get_all_node_properties_with_labels, node_label)))
                    label_dict[node_label] = {}
                    label_dict[node_label]['properties'] = sorted(node_properties_labels)
                    label_dict[node_label]['outgoing_relations'] = sorted([item for item in all_relationships if item.startswith(node_label)])
                    label_dict[node_label]['ingoing_relations'] = sorted([item for item in all_relationships if item.endswith(node_label) if not item.startswith(node_label)])
        ...
        def visualize_graph(g):
            ...
            for u, v, data in g.edges(data=True):
                rel_type = data['label']
                edge_color = choice(
                    edge_colors)  # Pick a random color for each relationship
            ...
            components.html(html_file.read(), height=435)
        ...
        st.error(f"Failed to connect to Neo4j: {str(ex)}")
        raise ex
